...
let bloco2 = new Sprite(300,300,100,50,'#0f0');
sprites.push(bloco2);
blocos.push(bloco2);
...
function movimenta(){
    ...
    personagem.posX = Math.max(0,Math.min(canvas.width-personagem.width,personagem.posX));//se posX for menor que 0 ele retorna o 0;
    //se posX for maior que tamanho do canvas - personagem.width retorna (canvas-p.width);
    personagem.posY = Math.max(0,Math.min((canvas.height-personagem.height),personagem.posY));
